...
public class MathExpr {
	public static void main(String[] args) {
		...
		/*
		String str = "23+ 2";
		double r = Calculate(ReversePolish(str));
		boolean k = Double.isNaN(r);
		System.out.println("等式是否无效:" + k);
		System.out.println("结果为:" + Math.round(r));
		*/
    		...
    		if (Character.isWhitespace(charArray[i])) {
    			...
    			if (BlankLength == 0) {
    				...
    				if (BlankStarts>0&& BlankEnds<(charArray.length-1)) {
    					...
    					if (((SInvalid=='.')||(Character.isDigit(SInvalid)))&&((EInvalid=='.')||(Character.isDigit(EInvalid)))) {
    						return Result;
    					}
    				}
    			}
					...
					if (charArray[i] == '+'||charArray[i] == '-') {
						...
						if (i < (charArray.length-5)) {
							if (charArray[i]=='s'&&charArray[i+1]=='i'&&charArray[i+2]=='n') {
								...
								if (charArray[i]=='s'&&charArray[i+1]=='q'&&charArray[i+2]=='r'&&charArray[i+3]=='t') {
									temp[i] = "sqrt" + "3";
									i = i + 3;
							} else {
								return Result;
			...
			if (temp[i] != null) {
				if (Character.isDigit(temp[i].charAt(0))) {
					...
					if (OperatorStack.isEmpty()) {
						...
						int operator2 = Integer.parseInt(OperatorStack.peek().substring(OperatorStack.peek().length()-1));
						if (operator1 == 4 || operator1 > operator2) {
							...
							while(operator1<=operator2&&operator2!=4){
								...
								if (!OperatorStack.isEmpty()) {
									operator2=Integer.parseInt(OperatorStack.peek().substring(OperatorStack.peek().length()-1));
								} else {
									break;
								}
							}
